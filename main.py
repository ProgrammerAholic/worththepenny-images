...
def get_soup(url):
    html = requests.get(url,headers={'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_4) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/35.0.1916.47 Safari/537.36', "Referer": "https://www.google.com"})
    soup = BeautifulSoup(html.content, "html.parser")
    return soup

def removeDubplicates():

    old_stores_list_json = None
    with open("worththepenny-images/stores-list-storage.json", "r") as f:
    ...
    print('---------------------------------- Start Comparing And Downloading Images... ----------------------------------')
    ...
    def find_unique_by_image(list1, list2):
        ...
        unique_items = [item for item in list2 if item['image_paths_to_download'] not in images_in_list1]
    ...
    if (len(find_unique_by_image) > 0):
        ...
        with open("worththepenny-images/stores-list-storage.json", "w") as file:
            json.dump(old_stores_list_json, file)
            file.close()

        print(f"-------------- Found {len(find_unique_by_image)} new image(s) ---------------")
...
def download_image(store, stores_array):
    ...
    dubplicate_items = [item for item in stores_array if item['store_alias'] == store['store_alias']]
    count_dubplicate_items = len(dubplicate_items)

    folder_path = "worththepenny-images/images"
    try:
        ...
        if response.status_code == 200:
            ...
            image_index = count_dubplicate_items + 1
            ...
            file_path = os.path.join(folder_path, file_name_save_to_folder + file_extension)       
        ...
        if os.path.exists("worththepenny-images/errors.txt"):
            with open("worththepenny-images/errors.txt", 'a') as link_error_file:
    ...
    while True:
        try:
            ...
            for store in all_store_boxes:
                ...
                info = {
                    "store_alias": store.find("a", class_="p_top worthepennycom")['href'].split("//")[1].split('.')[0] if store.find("a", class_="p_top worthepennycom")['href'] else "",
                    "image_paths_to_download": store.find("div", class_="_img_box p_image worthepennycom").find("img", class_="lazyload worthepennycom")['data-src'] if store.find("div", class_="_img_box p_image worthepennycom").find("img", class_="lazyload worthepennycom")['data-src'] else "",
                    "discount_percent": store.find("div", class_="p_text worthepennycom").find_all("p", class_="worthepennycom")[0].find("span").text.strip() if store.find("div", class_="p_text worthepennycom").find_all("p", class_="worthepennycom")[0] else "SALE",
                    "code": store.find("div", class_="p_text worthepennycom").find_all("p", class_="worthepennycom")[1].text.strip().split("Code: ")[1] if store.find("div", class_="p_text worthepennycom").find_all("p", class_="worthepennycom")[1] else "",
                    "link_store_details": store.find("a", class_="p_top worthepennycom")['href'] if store.find("a", class_="p_top worthepennycom")['href'] else ""
...
#####--------> Crawls all page paginations data, if the current page has no div element, break the loop
